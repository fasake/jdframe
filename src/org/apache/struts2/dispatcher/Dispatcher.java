...
public class Dispatcher {
    ...
    private static final org.apache.log4j.Logger LOG = org.apache.log4j.Logger.getLogger(Dispatcher.class);
    /**
    ...
    public void cleanup() {
        ...
        if (objectFactory instanceof ObjectFactoryDestroyable) {
            try {
            	...
            	e.printStackTrace();
                LOG.error("exception occurred while destroying ObjectFactory [#0]");
            }
        }
    ...
    public void serviceAction(HttpServletRequest request, HttpServletResponse response, ServletContext context,
        ...
        try {
            ...
            Object obj = request.getSession().getAttribute(Tokens._USER_PROFILE);
            String operator = "0";
            if(obj!=null){
            	UserProfile profile = (UserProfile) obj;
            	operator = profile.getUser().getUser_dm();
            	DbUtils.recodeLogToDB("DISPATCHER",new  BigDecimal(operator), "#"+serial_number+"#Request: \r\t\nnamespace:"+namespace+" \r\t\nname:"+name+" \r\t\nmethod:"+method);
            }else{
            	DbUtils.recodeLogToDB("DISPATCHER",new  BigDecimal(operator), "#"+serial_number+"#Request: \r\t\nnamespace:"+namespace+" \r\t\nname:"+name+" \r\t\nmethod:"+method+" ::"+request.getRemoteAddr()+"@"+request.getRemoteUser());
            }
            ...
            boolean executeResult = proxy.getExecuteResult();
            String actionNamespace = proxy.getNamespace();
            String resultCode = proxy.getInvocation().getResultCode();
            DbUtils.recodeLogToDB("DISPATCHER",new  BigDecimal(operator), "#"+serial_number+"#Proxy: \r\t\nexecuteResult:"+executeResult+" \r\t\nresultCode:"+resultCode+" \r\t\nactionNamespace:"+actionNamespace+" \r\t\nactionName:"+actionName+" \r\t\nmethodName:"+methodName);
    ...
    public void sendError(HttpServletRequest request, HttpServletResponse response, ServletContext ctx, int code, Exception e) {
        ...
        if (devModeOverride != null ? devModeOverride : devMode) {
           // if (LOG.isDebugEnabled()) {
        	    e.printStackTrace();
                LOG.debug("Exception occurred during processing request: #0");
           // }
            try {
            ...
            try {
               // if (LOG.isErrorEnabled()) {
            	    e.printStackTrace();
                    LOG.error("Exception occurred during processing request: #0");
               // }
